...
nuagehxi <- function(x){ #mÃ©thode matisse
  lastElem <- length(x)
...
displayFixedWidth <- function(x) {
  ...
  hist = hist(x, prob=T, breaks=seq(a0,ak,h), xlim=c(a0,ak), main="Histogramme à largeur fixe")
...
displayFixedEffec <- function(x) {
  ...
  print(res)
  hist = hist(x, prob=T, breaks=res, main="Histogramme Ã  effectif fixÃ©")
  lines(hist$mids, hist$density, lwd=3, col="Red")
  lines(density(x), lwd=3, col="Blue")
  plot(ecdf(x), main="Fonction de rÃ©partition empirique pour effectif fixÃ©")
}
